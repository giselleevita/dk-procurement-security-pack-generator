...
- `report.pdf`
- `evidence-pack.zip`
- `pack_manifest.json` (SHA-256 hashes for top-level pack files)
- `pack_manifest.sig` (signature over `pack_manifest.json`)
...
### Evidence integrity (tamper-evident)
- The pack includes a deterministic `pack_manifest.json` with SHA-256 hashes for `report.md`, `report.pdf`, and `evidence-pack.zip`.
- `pack_manifest.sig` signs the canonical JSON bytes of `pack_manifest.json` using the local instance signing key (Ed25519 when available; HMAC fallback).
- Verification endpoint: `GET /api/exports/{export_id}/verify` recomputes hashes and verifies the signature for export packs generated by this instance.
- Limitation: this proves integrity relative to the local instance trust anchor (self-hosted). If the signing key is rotated/recreated, older packs may no longer verify against the new key.
